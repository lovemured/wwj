...
"""删除市场活动 - 优先删除最早创建的
使用:
  python3 delete_market.py --id 活动ID
  python3 delete_market.py --count 5"""
...
def main():
    cfg=load_config()
    p=argparse.ArgumentParser(description='删除市场活动')
    ...
    if a.id:
        r=requests.delete(f'{pc}/api/pc/market_activities/{a.id}',headers=H)
        res=r.json()
    ...
    if a.count:
        ...
        while deleted<a.count:
            r=requests.get(f'{pc}/api/pc/market_activities?per_page=50&sort=created_at&order=asc&page={page}',headers=H)
            items=r.json().get('data',{}).get('list',[])
            if not items: break
            for c in items:
                if deleted>=a.count: break; pid=c['id']
                dr=requests.delete(f'{pc}/api/pc/market_activities/{pid}',headers=H)
