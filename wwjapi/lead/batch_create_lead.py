#!/usr/bin/env python3
"""批量创建线索(含附件) - 支持任意CRM环境
使用: python3 batch_create_lead.py --api https://域名 --token xxx [数量]"""
import requests,json,random,string,sys,argparse,time,os,tempfile
...
def build(info):
    ...
    c["department"]="研发部"
    c["job"]=random.choice(["经理","总监","主管","工程师","销售代表"])
    if info["labels"]: c["customer_labels_attributes"]=random.sample(info["labels"],min(3,len(info["labels"])))
    if info["market_activities"]: c["market_activity_id"]=random.choice(info["market_activities"])["id"]
    pm=rp()
    ...
    c["address_attributes"]={**region,"detail_address":f"{random.choice(ROADS)}{random.randint(1,999)}号","phone":pm,
...
def main():
    ...
    for i in range(1,a.count+1):
        try:
            data=build(info)
            data["name"]=f"批量线索-{datetime.now().strftime('%H%M%S')}-{i}"
            data["company_name"]=f"深圳{rt()}科技"
            data["note"]=f"批量第{i}条"
            nfiles=len(process_file_fields(a.api,a.token,"Lead",data,a.attachment_dir))
            pc=bool(info["fields"]["file"] and nfiles)
            url=pc_url(a.api)+"/api/pc/leads" if pc else f"{a.api}/api/v2/leads"
            r=requests.post(url,headers={"Content-Type":"application/json","Authorization":f"Token token={a.token}"},json={"lead":data},timeout=30)
            res=r.json()
