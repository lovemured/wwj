...
def main():
    ...
    r=requests.get(f"{api}/api/v2/quotations/{a.id}",headers={'Authorization':f'Token token={token}'})
    old=r.json().get('data',{}); 
    if not old.get('id'): print('报价单不存在'); return
    print('自动发现字段...')
    cf=fetch(api,'custom_fields?model_klass=Quotation',token,pc=True)
    info={'fields':{},'opts':{}}
    ...
    for g in cf.get('data',{}).get('custom_field_groups',[]):
        for f in g.get('custom_fields',[]):
            ...
            if n in ['name','quotation_no','quotation_date','effective_date_fr','effective_date_to','product_assets','product_total_amount','additional_discount_amount','whole_discount','total_amount','status','title','sub_title','note','revisit_remind_at','attachments']: continue
    ...
    fm=fetch(api,'field_maps/quotation',token)
    info['fm_opts']={}
    ...
    data['name']='报价单-'+rt(); data['quotation_no']='QTE'+rt()
    data['quotation_date']=rf(); data['effective_date_fr']=rf(); data['effective_date_to']=rf()
    data['total_amount']=rd(); data['product_total_amount']=rd()
    data['additional_discount_amount']=rd(); data['whole_discount']=ri()%100
    ...
    r=requests.put(f"{api}/api/v2/quotations/{a.id}",headers={'Content-Type':'application/json','Authorization':f'Token token={token}'},json={'quotation':data})
    res=r.json()
