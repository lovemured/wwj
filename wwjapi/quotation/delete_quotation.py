...
def main():
    ...
    if a.id:
        r=requests.delete(f'{v2}/quotations/{a.id}',headers=H)
        res=r.json()
    ...
    if a.count:
        ...
        while deleted<a.count:
            r=requests.get(f'{v2}/quotations?per_page=50&sort=created_at&order=asc&page={page}',headers=H)
            items=r.json().get('data',{}).get('quotations',[])
            if not items: break
            for c in items:
                if deleted>=a.count: break
                cid=c['id']
                dr=requests.delete(f'{v2}/quotations/{cid}',headers=H)
                if dr.json().get('code')==0:
                    deleted+=1; print(f"  ✓ [{deleted}/{a.count}] ID:{cid}")
