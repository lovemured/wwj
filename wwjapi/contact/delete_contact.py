...
def main():
    ...
    a=p.parse_args(); api=a.api.rstrip('/'); H={'Authorization':f'Token token={a.token}'}
    if a.id:
        r=requests.delete(f"{api}/api/v2/contacts/{a.id}",headers=H); res=r.json()
    ...
    if a.count:
        ...
        while deleted<a.count:
            r=requests.get(f"{api}/api/v2/contacts?per_page=50&sort=created_at&order=asc&page={page}",headers=H)
            items=r.json().get('data',{}).get('contacts',[])
            if not items: break
            for c in items:
                if deleted>=a.count: break; cid=c['id']
                dr=requests.delete(f"{api}/api/v2/contacts/{cid}",headers=H)
